...
class VGG:
    ...
    def build_model(self, input_shape, nb_classes):
        ...
        conv2 = keras.layers.Conv1D(filters=128, kernel_size=3, padding='same', activation='relu')(conv1)
        conv2 = keras.layers.Conv1D(filters=128, kernel_size=3, padding='same', activation='relu')(conv2)
        ...
        conv3 = keras.layers.Conv1D(filters=256, kernel_size=3, padding='same', activation='relu')(conv3)
        conv3 = keras.layers.Conv1D(filters=256, kernel_size=3, padding='same', activation='relu')(conv3)
        conv3 = keras.layers.MaxPooling1D()(conv3)

        '''
        conv4 = keras.layers.Conv1D(filters=512, kernel_size=3, padding='same', activation='relu')(conv3)
        conv4 = keras.layers.Conv1D(filters=512, kernel_size=3, padding='same', activation='relu')(conv4)
        conv4 = keras.layers.Conv1D(filters=512, kernel_size=3, padding='same', activation='relu')(conv4)
        conv4 = keras.layers.MaxPooling1D()(conv4)

        conv5 = keras.layers.Conv1D(filters=512, kernel_size=3, padding='same', activation='relu')(conv4)
        conv5 = keras.layers.Conv1D(filters=512, kernel_size=3, padding='same', activation='relu')(conv5)
        conv5 = keras.layers.Conv1D(filters=512, kernel_size=3, padding='same', activation='relu')(conv5)
        conv5 = keras.layers.MaxPooling1D()(conv5)

        flatten_layer = keras.layers.Flatten()(conv5)
        '''
